...
function renderLineup(){const models=new Map(state.data.models.map(m=>[m.name,m]));$('#lineup').innerHTML=state.data.roles.map(r=>{const name=LINEUP[r.id],m=models.get(name);return `<div class="lineup-item"><span class="lineup-role">${esc(r.label)}</span><strong>${esc(name)}</strong>${m?`<span class="lineup-sub">${fmt(m.roleScores?.[r.id]?.rankingQuality)} quality</span>`:''}</div>`}).join('')}
