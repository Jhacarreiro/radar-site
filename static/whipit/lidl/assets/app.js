...
function openAvailability(card){
  currentProduct={title:card.dataset.productTitle,brand:card.dataset.productBrand,price:card.dataset.productPrice,date:card.dataset.productDateLabel,unit:card.dataset.productUnit,url:card.dataset.productUrl,storeStockId:card.dataset.storeStockId};
  modal.hidden=false;document.body.classList.add('modal-open');
  storeSearch.value=''; userPos=null; list.innerHTML='';
  renderProductHeader();
  statusBox.textContent=currentProduct.storeStockId?'Usa localização ou pesquisa para verificar até 5 lojas.':'Este produto não tem identificador de disponibilidade.';
  loadStores();
}
...
function availabilityMeta(indicator){
  ...
  if(v==='AVAILABLE') return {bars:'▰▰▰',label:'Melhor aposta',cls:'available'};
...
async function fetchAvailability(stores){
  if(!currentProduct||!currentProduct.storeStockId) return new Map();
  ...
  const url=`https://www.lidl.pt/p/api/storestock/PT/pt/${encodeURIComponent(currentProduct.storeStockId)}?storeids=${encodeURIComponent(ids)}`;
...
function candidateStores(){
  ...
  if(term) rows=rows.filter(s=>`${s.city||''} ${s.street||''} ${s.zip||''}`.toLowerCase().includes(term));
...
async function verifyStores(reason){
  await loadStores();
  ...
  try{
    ...
    list.innerHTML=rows.map(s=>storeCard(s,map.get(lidlStoreId(s.id)),false)).join('');
    statusBox.textContent=userPos?`${rows.length} lojas mais próximas`:`${rows.length} lojas para “${term}”`;
  }catch(e){
...
storeSearch&&storeSearch.addEventListener('input',()=>{clearTimeout(availabilityTimer);availabilityTimer=setTimeout(()=>verifyStores('search'),350)});
useLocation&&useLocation.addEventListener('click',()=>{if(!navigator.geolocation){statusBox.textContent='Localização não disponível neste browser.';return}statusBox.textContent='A pedir localização…';navigator.geolocation.getCurrentPosition(pos=>{userPos={lat:pos.coords.latitude,lon:pos.coords.longitude};verifyStores('location')},()=>{statusBox.textContent='Não consegui obter localização. Podes escrever cidade ou rua.'},{enableHighAccuracy:false,timeout:10000,maximumAge:300000})});
