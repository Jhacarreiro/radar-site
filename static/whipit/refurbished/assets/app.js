...
function offerCard(o){
  const warnings = (o.warnings || []).slice(0,2).map(w => `<span class="badge warn">⚠️ ${escapeHtml(w)}</span>`).join('');
  return `<article class="card" data-offer-card><div class="card-top"><div><h3>${escapeHtml(o.title || 'Oferta')}</h3><div class="meta">${escapeHtml(o.store || '')} · ${escapeHtml(o.condition || 'estado n/d')} · ${o.warranty_months || '—'} meses garantia</div></div><span class="badge ${verdictClass(o.verdict)}">${verdictLabel(o.verdict)}</span></div><div class="badges"><span class="badge">${escapeHtml(o.category_label || '')}</span><span class="badge">confiança ${escapeHtml(o.match_confidence || 'low')}</span>${warnings}</div><div class="price-row"><div class="price-box"><small>Recond.</small><strong>${eur(o.refurbished_price)}</strong></div><div class="price-box"><small>Novo</small><strong>${eur(o.best_new_price)}</strong></div><div class="price-box"><small>Poupança</small><strong>${eur(o.saving_eur)} · ${o.saving_percent || 0}%</strong></div></div><div class="meta">Score ${o.score || 0}/100 · ${escapeHtml(o.seller || 'vendedor n/d')}</div><div class="actions"><a class="button primary" target="_blank" rel="nofollow noopener" href="${escapeAttr(o.listing_url || '#')}">Ver recondicionado</a><a class="button" target="_blank" rel="nofollow noopener" href="${escapeAttr(o.url_pt || '#')}">Validar PT</a><a class="button" target="_blank" rel="nofollow noopener" href="${escapeAttr(o.url_amz || '#')}">Amazon.es</a></div></article>`;
}
