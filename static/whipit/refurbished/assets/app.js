...
function shortTitle(s, max=68){s=String(s||'Oferta').trim(); return s.length > max ? s.slice(0, max-1).trim() + '…' : s;}
...
function offerCard(o){
  ...
  return `<article class="card" data-offer-card><div class="card-top"><div><h3 title="${escapeAttr(o.title || 'Oferta')}">${escapeHtml(shortTitle(o.title))}</h3><div class="meta">SKU: ${escapeHtml(skuOf(o))}</div></div>${warrantyBadge}</div><div class="badges"><span class="badge warn">⚠️ ${escapeHtml(note)}</span></div><div class="price-row"><div class="price-box"><small>Recond.</small><strong>${eur(o.refurbished_price)}</strong></div><div class="price-box"><small>Novo</small><strong>${eur(o.best_new_price)}</strong></div><div class="price-box"><small>Poupança</small><strong>${eur(o.saving_eur)} · ${o.saving_percent || 0}%</strong></div></div><div class="meta">Estado: ${escapeHtml(state)} · Vendido por ${escapeHtml(o.seller || o.store || 'loja original')}</div><div class="actions"><a class="button primary" target="_blank" rel="nofollow noopener" href="${escapeAttr(o.listing_url || '#')}">Ver artigo na PcComponentes</a><a class="button" target="_blank" rel="nofollow noopener" href="${escapeAttr(o.url_pt || '#')}">Comparar PT</a><a class="button" target="_blank" rel="nofollow noopener" href="${escapeAttr(o.url_amz || '#')}">Amazon.es</a></div></article>`;
}
